...
fn fs(input: SphereFragmentInput) -> FragmentOutput {
    ...
    out.frag_depth = clip_space_pos.z / clip_space_pos.w;    var diffuse: f32 = max(0.0, dot(normal, normalize(vec3(1.0, 1.0, 1.0))));

    // Add rim lighting for better 3D appearance
    var view_dir = normalize(-input.view_position);
    var rim = 1.0 - max(0.0, dot(normal, view_dir));
    var rim_light = pow(rim, 2.0) * 0.3;

    // Combine ambient, diffuse, and rim lighting
    var lighting = 0.3 + diffuse * 0.7 + rim_light; // ambient + diffuse + rim

    var color: vec3f = value_to_color(input.speed / 1.5);

    out.frag_color = vec4(color * lighting, 1.);
    return out;
}
